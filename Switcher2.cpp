// Switcher2 is a clone of Switcher by O.Goma adding milliseconds as the first option.

#include "Switcher2.h"


Switcher2::Switcher2(int pin, bool initstate)
...
int Switcher2::Period(long on, long off, int unit)  //Switch the relay according to the ON and OFF time set. Is referenced in Hours. 
  {
...
void Switcher2::Start()			//Switch the relay state. Turns to HIGH if it was OFF, OFF is it was ON 
  {
	if (st == _state) {
		st = !_state;
...
void Switcher2::Stop()			//Switch the relay state. Turns to HIGH if it was OFF, OFF is it was ON 
  {
	if (st != _state) {
		st = _state;
...
int Switcher2::Timer(long on, int unit)		//Switch the relay to initial state if it was switched more than x Time. It can be refferenced in seconds, minutes or hours.
  {
    unsigned long OnTime;
    unsigned long currentMillis = millis();
    int i = 0;
