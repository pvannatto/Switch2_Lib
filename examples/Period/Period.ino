 ...
 */
#include <Switcher2.h>       //Include Switcher library
#define LED 13               // Define the led's pin
bool LED_INIT = LOW;        //We define LOW as the INITIAL state of the led

Switcher2 led(LED, LED_INIT);    // Define the Switcher led function led(LED pin, pin initial state: HIGH or LOW )
...
void loop()
{
    led.Period(5,10,1);       // Led will stay ON for 5 second, OFF for 10 seconds     
}   
//  led.Period(5,10,2);        // We could use minutes: Led will stay on for 5 minute, off for 10 minutes 
//  led.Period(5,10,3);        // We could use hours: Led will stay on for 5 hour, off for 10 hours 
