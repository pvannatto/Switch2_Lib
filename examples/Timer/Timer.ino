...
Switcher2 led(LED, LED_INIT);   // Define the Switcher led function led( LED pin , initial state: HIGH or LOW )
...
void loop()
{
  led.Timer(5,1);               // Led will stay ON for 5 seconds, then it will turn OFF
  if (condition = 0) {
    led.Start();                // It will turn ON the led
    condition = 1;
  }
}
//  led.Timer(5,2);   // Led will stay ON for 5 minute
//  led.Timer(5,3);   // Led will stay ON for 5 hour
